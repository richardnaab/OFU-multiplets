...
class multiplets():
    ...
    #find all multiplets with multiplicity m
        ...
        while np.any( time_diff<self._delta_t ):
            ...
            #print(np.where(space_diff<self._delta_ang))
            #print(np.where(time_diff<self._delta_t))
            #print('n_lag: ',n_lag)
            ...
            if np.any(total_cut):
                ...
                if first_events == True:
                    ...
                    first_events = False
                    #print('Events: ',event1,event2)
                else: # insert at correct times
                    #print('insert events')
                    #print('Before: # events: ',len(event1))
                    #print(self.data[:-n_lag][total_cut])
                    #print(event1['time'],self.data['time'][:-n_lag][total_cut])
